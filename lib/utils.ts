...
export const getConstraintsFromValidation = (schema: ObjectSchema<any>, field: string, constraintName: string) => {
  const fieldRule: any = (schema.describe().fields[field] as SchemaDescription).tests.find((rule) => rule.name === constraintName);
